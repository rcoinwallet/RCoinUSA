...
// RCoinUSA: optional setting to unlock wallet for block minting only;
...
// RCoinUSA: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // RCoinUSA: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // RCoinUSA: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // RCoinUSA: not avatar mode
                        {
...
// RCoinUSA: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    CBigNum bnTargetPeRCoinUSADay;
    bnTargetPeRCoinUSADay.SetCompact(nBits);
...
// RCoinUSA: check 'spent' consistency between wallet and txindex
// RCoinUSA: fix wallet spent state according to txindex
...
// RCoinUSA: disable transaction (only for coinstake)
